...
local casino_chips_current_limit = casino_chips:new_text(function ()
    return 'Current chips purchase limit: '..script.get_global(262145):at(27238):read_u32()
end)
local casino_chips_choose_limit = casino_chips:new_slider('Chips purchase limit', 50000, 2000000)
local casino_chips_set_limit = casino_chips:new_button('Set chips purchase limit', function ()
    script.get_global(262145):at(27238):write_u32(casino_chips_choose_limit:get())
    ui.popup('GOD SYSTEM', 'New limit: '..casino_chips_choose_limit:get()..'.', Icons.CHECKMARK_SUCCESS, PopupType.BOX)
end)

local casino_chips_reset_cooldown = casino_chips:new_button('Reset chips purchase cooldown', function ()
...
local casino_bj_trickdealer = casino_bj:new_button('Trick the dealer', function ()
    if script.is_running('blackjack') then
        local current_table = script.get_local('blackjack', 1774 + 1 + (Pid() * 8) + 4):read_u32()
...
local casino_poker_flush = casino_poker:new_button('Give straight flush', function ()
    if script.is_running('three_card_poker') then
        local current_table = script.get_local('three_card_poker', 747 + 1 + (Pid() * 9) + 2):read_u32()
...
local casino_poker_trickthedealer = casino_poker:new_button('Trick the dealer', function ()
    if script.is_running('three_card_poker') then
        local current_table = script.get_local('blackjack', 747 + 1 + (Pid() * 9) + 2):read_u32()
        if current_table ~= nil and current_table ~= -1 then
