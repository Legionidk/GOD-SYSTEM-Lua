...
end):setHint('Unlocks 197 units of previously removed vehicles from stores.', '197')
...
Trash:add_click_option('TP to gun van', 'HC_Stuff_GV', function ()
    if not check_table(script_global:new(2652572 + 2650 + 1):get_int64(), Gunvan_coords, 1) then
        notify.warning('gv_error', 'Error: couldnt find gun van')
    else
