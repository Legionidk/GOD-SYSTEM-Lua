...
task.createTask = function (hash_s, delay_n, count_n, callback_f)
    if task.exists(hash_s) then
        return log.error("TASKS", string.format("Failed to create task with name: %s | Task already exists.", hash_s))
    end
...
task.removeTask = function (hash_s)
    ...
    log.error("TASKS", string.format("Failed to remove task with name \'%s\' | Task doesnt exist.", hash_s))
    return true
end
...
task.setCallback = function (hash_s, callback_f)
    ...
    log.error("TASKS", string.format("Failed to set callback for task with name \'%s\' | Task doesnt exist.", hash_s))
    return true
end
...
task.setDelay = function (hash_s, delay_n)
    ...
    log.error("TASKS", string.format("Failed to set delay for task with name \'%s\' | Task doesnt exist.", hash_s))
    return true
end
...
task.setCount = function (hash_s, count_n)
    ...
    log.error("TASKS", string.format("Failed to set count for task with name \'%s\' | Task doesnt exist.", hash_s))
    return true
end
