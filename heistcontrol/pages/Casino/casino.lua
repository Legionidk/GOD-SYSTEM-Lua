...
Main:add_sub_option('Настройка ограбления', 'HC_CasinoSettings', CasinoSettings)
...
CasinoSettings:add_choose_option('Подход', 'HC_CasinoSettings_Approach', false, {'Скрытность', 'Обман', 'Штурм'}, function (pos)
    ...
    stats_set('MP'..player_index..'_H3_LAST_APPROACH', last_approach[pos])
    stats_set('MP'..player_index..'_H3OPT_APPROACH', pos)
...
CasinoSettings:add_choose_option('Главная цель', 'HC_CasinoSettings_Main', false, {'Деньги', 'Золото', 'Картины', 'Алмазы'}, function (pos)
    ...
    player_index = script_global:new(1574915):get_int64()
    stats_set('MP'..player_index..'_H3OPT_TARGET', main_numbers[pos])
    notify.success('Casino', 'Успешно выбрано: ' ..main_position[pos])
end)
...
CasinoSettings:add_choose_option('Стрелок', 'HC_CasinoSettings_Gunnman', false, {'Карл Аболаджи', 'Густаво Мота', 'Чарли Рид', 'Честер Маккой', 'Патрик Макрири', 'Не выбран'}, function (pos)
    ...
    player_index = script_global:new(1574915):get_int64()
    if pos == 6 then
        stats_set('MP'..player_index..'_H3OPT_CREWWEAP', 0)
    else
        stats_set('MP'..player_index..'_H3OPT_CREWWEAP', pos)
    end
    notify.success('Casino', 'Успешно выбрано: ' ..main_position[pos])
end)
...
CasinoSettings:add_choose_option('Водитель', 'HC_CasinoSettings_Driver', false, {'Карим Денц', 'Талиана Мартинес', 'Эдди Тоу', 'Зак Нельсон', 'Честер Маккой', 'Не выбран'}, function (pos)
    ...
    player_index = script_global:new(1574915):get_int64()
    if pos == 6 then
        stats_set('MP'..player_index..'_H3OPT_CREWDRIVER', 0)
    else
        stats_set('MP'..player_index..'_H3OPT_CREWDRIVER', pos)
    end
    notify.success('Casino', 'Успешно выбрано: ' ..main_position[pos])
end)
...
CasinoSettings:add_choose_option('Хакер', 'HC_CasinoSettings_Hacker', false, {'Рикки Люкенс', 'Кристиан Фелти', 'Йохан Блэр', 'Ави Шварцман', 'Пейдж Харрис', 'Не выбран'}, function (pos)
    ...
    player_index = script_global:new(1574915):get_int64()
    if pos == 6 then
        stats_set('MP'..player_index..'_H3OPT_CREWHACKER', 0)
    else
        stats_set('MP'..player_index..'_H3OPT_CREWHACKER', pos)
    end
    notify.success('Casino', 'Успешно выбрано: ' ..main_position[pos])
end)
...
CasinoSettings:add_click_option('Выполнить подготовительные', 'HC_CasinoSettings_SkipPreps', function ()
    player_index = script_global:new(1574915):get_int64()
    stats_set('MP'..player_index..'_H3OPT_DISRUPTSHIP', 3)
    stats_set('MP'..player_index..'_H3OPT_KEYLEVELS', 2)
    stats_set('MP'..player_index..'_H3OPT_VEHS', 3)
    stats_set('MP'..player_index..'_H3OPT_WEAPS', 0)
    stats_set('MP'..player_index..'_H3OPT_BITSET0', -1)
    stats_set('MP'..player_index..'_H3OPT_BITSET1', -1)
...
CasinoSettings:add_click_option('Открыть все входы', 'HC_CasinoSettings_OpenPoi', function ()
    player_index = script_global:new(1574915):get_int64()
    stats_set('MP'..player_index..'_H3OPT_POI', -1)
    stats_set('MP'..player_index..'_H3OPT_ACCESSPOINTS', -1)
    notify.success('Casino', 'Все входы открыты')
end)
...
Main:add_click_option('Быстрая настройка ограбления', 'HC_CasinoFast', function ()
    player_index = script_global:new(1574915):get_int64()
    stats_set('MP'..player_index..'_H3_LAST_APPROACH', 3) -- Обман
    stats_set('MP'..player_index..'_H3OPT_APPROACH', 2) -- Обман
    stats_set('MP'..player_index..'_H3OPT_TARGET', 3) -- Алмазы 
    stats_set('MP'..player_index..'_H3OPT_CREWWEAP', 1) -- Стрелок Карл Аболаджи (5%)
    stats_set('MP'..player_index..'_H3OPT_CREWDRIVER', 1) -- Водитель Карим Денц (5%)
    stats_set('MP'..player_index..'_H3OPT_CREWHACKER', 1) -- Хакер Рикки Люкенс (3%)
    stats_set('MP'..player_index..'_H3OPT_DISRUPTSHIP', 3) -- Подготовительные
    stats_set('MP'..player_index..'_H3OPT_KEYLEVELS', 2) -- Подготовительные
    stats_set('MP'..player_index..'_H3OPT_VEHS', 3) -- Подготовительные
    stats_set('MP'..player_index..'_H3OPT_WEAPS', 0) -- Подготовительные
    stats_set('MP'..player_index..'_H3OPT_BITSET0', -1) -- Подготовительные
    stats_set('MP'..player_index..'_H3OPT_BITSET1', -1) -- Подготовительные
    stats_set('MP'..player_index..'_H3OPT_POI', -1) -- Входы
    stats_set('MP'..player_index..'_H3OPT_ACCESSPOINTS', -1) -- Входы
    notify.success('Casino', 'Ограбление успешно настроено')
...
Main:add_click_option('Сбросить ограбление', 'HC_CasinoSettings_Reset', function ()
    player_index = script_global:new(1574915):get_int64()
    stats_set('MP'..player_index..'_H3_LAST_APPROACH', 0)
    stats_set('MP'..player_index..'_H3_HARD_APPROACH', 0)
    stats_set('MP'..player_index..'_H3OPT_APPROACH', 0)
    stats_set('MP'..player_index..'_H3OPT_TARGET', 0)
    stats_set('MP'..player_index..'_H3OPT_CREWWEAP', 0)
    stats_set('MP'..player_index..'_H3OPT_CREWDRIVER', 0)
    stats_set('MP'..player_index..'_H3OPT_CREWHACKER', 0)
    stats_set('MP'..player_index..'_H3OPT_DISRUPTSHIP', 0)
    stats_set('MP'..player_index..'_H3OPT_KEYLEVELS', 0)
    stats_set('MP'..player_index..'_H3OPT_VEHS', 0)
    stats_set('MP'..player_index..'_H3OPT_WEAPS', 0)
    stats_set('MP'..player_index..'_H3OPT_BITSET0', 0)
    stats_set('MP'..player_index..'_H3OPT_BITSET1', 0)
    stats_set('MP'..player_index..'_H3OPT_POI', 0)
    stats_set('MP'..player_index..'_H3OPT_ACCESSPOINTS', 0)
