local paths = require("heistcontrol/globals/paths.lua")
require("heistcontrol/globals/stuff.lua")
local parse = require("heistcontrol/util/parse")
local fs = require("heistcontrol/util/file_system")
...
local config = {
    ...
    notifyOnHotkey = true,
    notify2Offset = 5,
    notify2AlignRight = true,
    notify2AlignTop = true,
}
...
materials.success = draw.create_texture_from_file(paths.files.imgs.success)
...
function NotifyService:notify2(text_s, r, g, b, iconID_n)
    ...
    local texture = nil
    switch (iconID_n, {
        [gui.drawUI_icons.success] = function ()
            texture = materials.success
        end,
        [gui.drawUI_icons.info] = function ()
            texture = materials.info
        end,
        [gui.drawUI_icons.warning] = function ()
            texture = materials.warning
        end,
        [gui.drawUI_icons.error] = function ()
            texture = materials.error
        end
    }, function ()
        texture = materials.info
    end)
    local notifyHash = "Notify_Render_" .. os.clock() .. math.random(1337)
    local textSize = draw.get_text_size(text_s)
    local notifyHeight = 10 + textSize.y + 10
    local notifyWidth = 24 + 10 + textSize.x + 10 + config.notify2Offset
    local yOffset = 20.0
    for _, height in pairs(config.notifies) do
        yOffset = yOffset + height + 10
    end
    ...
    listener.register(notifyHash, GET_EVENTS_LIST().OnFrame, function ()
        if os.clock() >= notifyTime then
            ...
            if targetAlpha == alpha then
                ...
                return listener.remove(notifyHash, GET_EVENTS_LIST().OnFrame)
            end
        end
        if alpha < targetAlpha then
            alpha = math.min(targetAlpha, alpha + math.ceil(step * utils.get_frame_rate()))
        elseif alpha > targetAlpha then
            alpha = math.max(targetAlpha, alpha - math.ceil(step * utils.get_frame_rate()))
        end
        local leftUpper = {
            x = config.notify2AlignRight and (draw.get_window_width() - notifyWidth - config.notify2Offset) or (0 + config.notify2Offset),
            y = config.notify2AlignTop and (0 + yOffset) or (draw.get_window_height() - yOffset - notifyHeight),
        }
        local rightDown = {
            x = leftUpper.x + notifyWidth + config.notify2Offset * (config.notify2AlignRight and -1 or 1),
        ...
        draw.texture(
            texture,
            leftUpper.x + 5,
            leftUpper.y + math.ceil(notifyHeight/2) - 24/2,
            24,
            24
        )
        draw.set_rounding(0)
...
function InputService:displayInputBox(name_s, type_s, callback_f)
    ...
    elseif type_s == "hotkey" then
        title = "Нажмите любую клавишу для установки хоткея."
    end
...
function Option.new(submenu_mt, name_s, hash_s, type_n, value_n, callback_f)
    ...
    option.callback = function (...)
        if not callback_f then return end
        local args = {...}
        task.executeAsScript(hash_s .. tostring(os.clock()) .. tostring(math.random(1337)), function ()
            try (function ()
                callback_f(table.unpack(args), option)
            end, function (error)
                log.error("DrawUI", "Failed to execute the callback! Option: {}\nError: {}", option.name, error)
                option.callback = nil
            end)
        end)
    end
...
function Submenu:add_looped_option(name_s, hash_s, delay_n, callback_f, on_finish_f)
    local option = self:add_bool_option(name_s, hash_s, function (state, option)
        if state then
            task.createTask(hash_s, delay_n, nil, function (count)
                callback_f(count, option)
            end)
        else
...
listener.register("DrawUI_render", GET_EVENTS_LIST().OnFrame, function ()
    ...
    if #submenu.options == 0 then
        ...
        notify.default("DrawUI", "Здесь пока ничего нет.")
    end
