...
notify = {
    default = function (page_s, text_s, ...)
        ...
        NotifyService:notify2(features.format(text_s, table.unpack({...})), 66, 153, 225, gui.drawUI_icons.info)
    end,
    success = function (page_s, text_s, ...)
        -- NotifyService:notify2(features.format(text_s, table.unpack({...})), 0, 204, 153)
        NotifyService:notify2(features.format(text_s, table.unpack({...})), 72, 187, 120, gui.drawUI_icons.success)
    end,
    warning = function (page_s, text_s, ...)
        -- NotifyService:notify(page_s, features.format(text_s, table.unpack({...})), 255, 204, 51)
        NotifyService:notify2(features.format(text_s, table.unpack({...})), 236, 201, 75, gui.drawUI_icons.warning)
    end,
    important = function (page_s, text_s, ...)
        -- NotifyService:notify(page_s, features.format(text_s, table.unpack({...})), 51, 102, 204)
        NotifyService:notify2(features.format(text_s, table.unpack({...})), 102, 153, 153, gui.drawUI_icons.warning)
    end,
    fatal = function (page_s, text_s, ...)
        -- NotifyService:notify(page_s, features.format(text_s, table.unpack({...})), 255, 0, 51)
        NotifyService:notify2(features.format(text_s, table.unpack({...})), 245, 101, 101, gui.drawUI_icons.error)
    end,
}